...
import { Plus, FileText, Zap, Globe, Code2, Shield, Calendar, Wifi } from "lucide-react";
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-1">
            ...
            <Select
              ...
              onValueChange={(value) =>
                setNewVariable((prev) => ({
                  ...prev,
                  type: value as "string" | "number" | "boolean" | "secret" | "environment" | "dynamic",
                ...
                <SelectContent className="max-h-[300px]">
                  {allGenerators.map((g) => (
                    <SelectItem key={g.name} value={g.name}>
                      <div className="flex items-center gap-2">
                        {g.category === "basic" && <Code2 className="w-4 h-4" />}
                        {g.category === "random" && <Zap className="w-4 h-4" />}
                        {g.category === "auth" && <Shield className="w-4 h-4" />}
                        {g.category === "date" && <Calendar className="w-4 h-4" />}
                        {g.category === "network" && <Wifi className="w-4 h-4" />}
                        <div>
              ...
              {newVariable.generatorFunction && (() => {
                ...
                return generator?.configSchema ? (
                  <div className="space-y-2 p-3 bg-muted/50 rounded-lg">
                    ...
                    {Object.entries(generator.configSchema).map(([key, schema]) => (
                          ...
                          onChange={(e) =>
                            setNewVariable((prev) => ({
                              ...
                              generatorConfig: {
                                ...
                                [key]: (schema as { type: string }).type === "number" ? Number(e.target.value) : e.target.value,
                              },
                            }))
                          }
                        />
                ...
                <div className="p-3 bg-blue-50 dark:bg-blue-950/30 rounded-lg">
                  ...
                  <div className="text-sm font-mono">
                    {(() => {
                      try {
                        const generator = getGenerator(newVariable.generatorFunction);
                        return generator?.generate(newVariable.generatorConfig) || "";
                      } catch (e) {
          ...
          <div className="space-y-1">
            ...
            <Select
              ...
              onValueChange={(value) =>
                setNewVariable((prev) => ({ ...prev, scope: value as "environment" | "global" | "project" | undefined }))
              }
            >
              <SelectTrigger>
                <SelectValue />
