...
export const Environments: React.FC = () => {
  ...
  const currentEnvironments = filtered.slice(
    startIndex,
  ...
  const handleCreateSave = async (payload: Partial<Environment>) => {
    ...
    try {
      ...
      toast({
        ...
        description: created.name || 'New environment created.',
      });
      setIsCreateOpen(false);
    } catch (e: any) {
      toast({
        title: 'Create failed',
        description: e?.message || 'Could not create environment.',
        variant: 'destructive',
      });
    } finally {
      setIsCreating(false);
  ...
  const handleEditSave = async (payload: Environment) => {
    ...
    try {
      ...
      toast({
        title: 'Update failed',
        description: e?.message || 'Could not update environment.',
        variant: 'destructive',
      });
    } finally {
  ...
  const handleDeleteEnv = async (id: string, label?: string) => {
    ...
    try {
      ...
      toast({
        ...
        description: `${label ?? 'Environment'} has been deleted.`,
        variant: 'destructive',
      });
    } catch (e: any) {
      setLocalList(prev);
      toast({
        title: 'Delete failed',
        description: e?.message || "We couldn't delete the environment.",
        variant: 'destructive',
      });
    } finally {
      ...
      <div className='flex items-center justify-between gap-3'>
        ...
        <div className='flex items-center gap-2'>
          <Button
            ...
            onClick={() => setLocation('/settings/account?tab=environments')}
          >
            Manage Environment
          </Button>
        </div>
