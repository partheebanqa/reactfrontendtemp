import {
  ...
  const DetailedTestResults = ({ categories }: { categories: TestCategory[] }) => {
    ...
    return (
      <div className="rounded-lg border border-gray-200 space-y-3 mt-8 p-5 rounded-lg bg-white">
         <h2 className="text-1xl font-bold text-foreground mb-1">Detailed Test Results</h2>
        {categories.map((cat) => {
              ...
              {isOpen && (
                <div className="divide-y border-t border-[#E0E0E0]">
                  {cat.tests.map((test, i) => (
                    <div
                      key={i}
                      className="flex items-center justify-between px-6 py-4 bg-white hover:bg-gray-50"
                    >
                      ...
                      <div className="text-right text-sm">
                        ...
                        <p className="text-muted-foreground text-xs">Status: {test.statusCode}</p>
                      </div>
                    </div>
                  ))}
                </div>
