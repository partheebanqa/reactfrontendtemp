...
import { DataExtraction, Variable } from '../../shared/types/chainRequestTypes';
...
export function ResponseExplorer({ 
  ...
  const parseJsonToNodes = (obj: any, parentPath = '', level = 0): JsonNode[] => {
  ...
  const handleExtractClick = (source: DataExtraction['source'], path: string, value: any) => {
    const suggestedName = path.split('.').pop()?.replace(/[\[\]]/g, '') || 'extractedValue';
    setExtractionModal({
      isOpen: true,
      source,
      path,
  ...
  const renderJsonValue = (node: JsonNode, isVisible: boolean) => {
    ...
    const isAlreadyExtracted = existingExtractions.some(e => e.path === node.path);
          ...
          <div className="flex items-center flex-1 min-w-0">
            ...
            {!hasChildren && (
              <span className={`text-sm font-mono ${
                ...
                node.type === 'number' ? 'text-purple-600' :
          ...
          {!hasChildren && (
            <div className="flex items-center space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
              ...
              {isAlreadyExtracted ? (
                ...
                <button
                  onClick={() => handleExtractClick('response_body', node.path, node.value)}
    ...
    try {
      ...
      return (
        <div className="p-4 bg-gray-50 rounded border">
          ...
          <pre className="mt-2 text-xs text-gray-700 whitespace-pre-wrap">{response.body}</pre>
        </div>
      );
    }
  };
  ...
  const renderHeadersTab = () => (
    <div className="space-y-2">
      {Object.entries(response.headers).map(([key, value]) => {
        const isAlreadyExtracted = existingExtractions.some(e => e.source === 'response_header' && e.path === key);
        
        return (
          <div key={key} className="group flex items-center justify-between p-3 border border-gray-200 rounded-lg hover:bg-gray-50">
            <div className="flex-1 min-w-0">
              <div className="flex items-center space-x-2">
                <Hash className="w-4 h-4 text-gray-400" />
                <span className="font-medium text-gray-900 text-sm">{key}</span>
              </div>
              <p className="text-sm text-gray-600 font-mono mt-1 break-all">{value}</p>
            </div>
            ...
            <div className="flex items-center space-x-2 ml-4">
              ...
              {isAlreadyExtracted ? (
                ...
                <button
                  onClick={() => handleExtractClick('response_header', key, value)}
    ...
    <div className="space-y-2">
      {response.cookies && Object.keys(response.cookies).length > 0 ? (
        Object.entries(response.cookies).map(([key, value]) => {
          const isAlreadyExtracted = existingExtractions.some(e => e.source === 'response_cookie' && e.path === key);
            ...
            <div key={key} className="group flex items-center justify-between p-3 border border-gray-200 rounded-lg hover:bg-gray-50">
              ...
              <div className="flex items-center space-x-2 ml-4">
                ...
                {isAlreadyExtracted ? (
                  ...
                  <button
                    onClick={() => handleExtractClick('response_cookie', key, value)}
                    className="px-3 py-1 bg-blue-600 text-white rounded text-sm hover:bg-blue-700 transition-colors"
                  >
                    <Plus className="w-4 h-4 mr-1 inline" />
                    Extract
        ...
        <div className="bg-green-50 border border-green-200 rounded-lg p-6">
          <div className="flex items-center justify-between mb-4">
            <h4 className="font-medium text-green-900 flex items-center space-x-2">
              ...
              <span>Extracted Variables ({Object.keys(extractedVariables).length})</span>
            </h4>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            {Object.entries(extractedVariables).map(([name, value]) => {
              const extraction = existingExtractions.find(e => e.variableName === name);
              return (
                <div key={name} className="bg-white border border-green-200 rounded-lg p-3">
                  ...
                  <div className="text-sm">
                    <p className="text-gray-600 mb-1">Path: <code className="bg-gray-100 px-1 rounded">{extraction?.path}</code></p>
          ...
          <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
            <p className="text-sm text-blue-800">
              <strong>💡 Usage:</strong> Use these variables in other requests with the syntax: <code className="bg-blue-100 px-1 rounded">{`{{variableName}}`}</code>
            </p>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md">
            <div className="p-6 border-b border-gray-200">
              <h3 className="text-lg font-semibold text-gray-900">Extract Variable</h3>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                <div className="flex items-center space-x-2 p-3 bg-gray-50 rounded-lg">
                  {extractionModal.source === 'response_body' && <Code className="w-4 h-4 text-blue-600" />}
                  {extractionModal.source === 'response_header' && <Hash className="w-4 h-4 text-blue-600" />}
                  {extractionModal.source === 'response_cookie' && <Cookie className="w-4 h-4 text-blue-600" />}
                  <span className="text-sm text-gray-700">
                    {extractionModal.source.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase())}
                  </span>
                </div>
              </div>
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  JavaScript expression to transform the value (use 'value' as variable)
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center justify-end space-x-3 p-6 border-t border-gray-200">
              ...
              <button
                onClick={() => {
                  const variableName = (document.getElementById('variableName') as HTMLInputElement).value;
                  const transform = (document.getElementById('transform') as HTMLInputElement).value;
                  if (variableName) {
