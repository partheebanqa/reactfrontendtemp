...
export function VariablesTable({
  ...
  console.log('chainVariables:', chainVariables);
    ...
    requests.forEach((request, requestIndex) => {
      ...
      request.extractVariables.forEach((extraction) => {
        ...
        if (
          !extractedValue &&
          chainVariables &&
          Array.isArray(chainVariables) &&
          chainVariables.length > 0
        ) {
          chainVariables.forEach((chainVar) => {
            if (chainVar && chainVar.extractedVariables) {
              try {
                const parsedVars = JSON.parse(chainVar.extractedVariables);
                if (Array.isArray(parsedVars)) {
                  const matchingVar = parsedVars.find(
                    (v: any) => v && v.name === variableName
                  );
                  if (matchingVar) {
                    extractedValue = matchingVar.value;
                  }
                }
              } catch (error) {
                console.error('Error parsing chain variables:', error);
              }
            }
          });
        }
                            ...
                            <code className='px-2 py-1 bg-gray-100 text-gray-800 rounded text-sm font-mono'>
                              ...
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <button
                                    onClick={() =>
                                      handleCopy(result.variableName)
                                    }
