...
import { ExecutionLog, APIRequest, DataExtraction } from '../../shared/types/chainRequestTypes';
...
export function VariablesTable({ 
  ...
  const [sourceFilter, setSourceFilter] = useState<'all' | 'response_body' | 'response_header' | 'response_cookie'>('all');
  const [sortBy, setSortBy] = useState<'request' | 'variable' | 'status' | 'timestamp'>('request');
  ...
  const extractionResults = useMemo(() => {
    ...
    requests.forEach((request, requestIndex) => {
      request.dataExtractions.forEach((extraction) => {
        ...
        const extractedValue = log?.extractedVariables?.[extraction.variableName];
        ...
        results.push({
          ...
          extractedValue: extractedValue || extractedVariables[extraction.variableName],
          status,
    ...
    return results;
  }, [requests, executionLogs, extractedVariables, isExecuting, currentRequestIndex]);
  ...
  const filteredResults = useMemo(() => {
    let filtered = extractionResults.filter(result => {
      ...
      const matchesRequest = requestFilter === 'all' || result.requestId === requestFilter;
      const matchesSource = sourceFilter === 'all' || result.extractionSource === sourceFilter;
    ...
    return filtered;
  }, [extractionResults, searchTerm, statusFilter, requestFilter, sourceFilter, sortBy, sortOrder]);
  ...
  const exportResults = () => {
    const csvContent = [
      ['Request', 'Variable Name', 'Source', 'Path', 'Value', 'Status', 'Timestamp'],
    ...
    <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
      ...
      <div className="p-6 border-b border-gray-200">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h3 className="text-lg font-semibold text-gray-900">Variables Table</h3>
              <p className="text-sm text-gray-500">Track all variable extractions across the chain</p>
            </div>
          </div>
              ...
              {filteredResults.map((result) => (
                <React.Fragment key={result.id}>
                  <tr className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center space-x-3">
                        <span className={`px-2 py-1 text-xs font-medium rounded ${getMethodColor(result.requestMethod)}`}>
                          {result.requestMethod}
                        </span>
                        <div>
                          <p className="text-sm font-medium text-gray-900">{result.requestName}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <div className="flex items-center space-x-2">
                        ...
                        <div>
                          <p className="text-sm text-gray-600">
                            {result.extractionSource.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase())}
                          </p>
                          <code className="text-xs text-gray-500 font-mono">{result.extractionPath}</code>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4">
                        ...
                        {result.extractedValue !== undefined ? (
                          <div className="flex items-center space-x-2">
                            <code className="px-2 py-1 bg-green-50 text-green-800 rounded text-sm font-mono truncate">
                              {typeof result.extractedValue === 'object' 
                                ? JSON.stringify(result.extractedValue).substring(0, 50) + '...'
                                : String(result.extractedValue).substring(0, 50) + (String(result.extractedValue).length > 50 ? '...' : '')
                              }
                            </code>
                            {(typeof result.extractedValue === 'object' || String(result.extractedValue).length > 50) && (
                              <button
                  ...
                  {/* Expanded Row */}
                  {expandedRows.has(result.id) && result.extractedValue !== undefined && (
                    <tr>
                      <td colSpan={7} className="px-6 py-4 bg-gray-50">
                        <div className="space-y-3">
                          <h4 className="font-medium text-gray-900">Full Value:</h4>
      ...
      {filteredResults.length > 0 && (
        <div className="px-6 py-3 bg-gray-50 border-t border-gray-200">
          <p className="text-sm text-gray-600">
            Showing {filteredResults.length} of {extractionResults.length} variable extractions
          </p>
        </div>
      )}
    </div>
