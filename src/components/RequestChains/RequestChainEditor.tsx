...
export function RequestChainEditor({
  ...
  useEffect(() => {
    ...
    if (lastSyncedChainUpdatedAt.current === chain.updatedAt) return;

    lastSyncedChainUpdatedAt.current = chain.updatedAt;

    setFormData({
      name: chain.name || '',
      description: chain.description || '',
      workspaceId: chain.workspaceId || currentWorkspace?.id || '',
      enabled: chain.enabled ?? true,
      chainRequests: (
        chain.chainRequests ||
        (chain as any)?.requests ||
        ...
        assertions: req.assertions || [],
        variables: req.variables || [],
        isSelected: req.isSelected !== false,
      })),
      variables: chain.variables || [],
      environment: chain.environment || 'dev',
      tags: chain.tags || [],
  ...
  // P1-D: persistAssertionsToStorage moved to useAssertionState hook
  // Hook is initialised here (after formData is available):
  const {
    assertionsByRequest,
    setAssertionsByRequest,
                                      ...
                                      >
                                        {expandedRequests.has(request.id) && (
                                          <div className='mt-4 pt-4 border-t space-y-4'>
                                            ...
                                            {/* P1-D: ExecutionResultPanel is memoized — only re-renders when its own log/assertions/vars change */}
                                            {executionLog && (
