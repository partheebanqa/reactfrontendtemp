...
type MenuAction = "execute" | "view" | "edit" | "duplicate" | "reports" | "delete";
...
const RequestChainsList: React.FC = () => {
    ...
    const filteredAndSortedChains = useMemo(() => {
        ...
        return list;
    }, [requestChains, searchQuery, environmentFilter, statusFilter, tagsFilter, sortOrder]);

    const itemsPerPage = 10;

    const totalPages = Math.max(1, Math.ceil(filteredAndSortedChains.length / itemsPerPage));
    ...
    const handleAction = (action: MenuAction, chain: RequestChain) => {
        // replace with your real handlers / navigation
        console.log(`${action}:`, chain.name);
        setOpenMenuId(null);
    };
        ...
        return (
            <div className="flex items-center justify-between px-4 py-4 bg-white border-t border-slate-200">
                <div className="text-sm text-slate-600">
                    Showing{" "}
                    {filteredAndSortedChains.length === 0 ? 0 : startIndex + 1} to{" "}
            ...
            <main className="mx-auto px-2 sm:px-2 lg:px-2 py-6">
                ...
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 mb-6">
                    ...
                    <button
                        type="button"
                        onClick={() => {
                            // replace with real API refresh if needed
                            console.log("refresh");
                        }}
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {currentChains.map((chain) => (
                                <tr key={chain.id} className="hover:bg-slate-50/50 transition-colors">
                                    <td className="px-6 py-4">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="max-w-xs">
                                                <p className="font-semibold text-slate-900 truncate">{chain.name}</p>
                                    ...
                                    <td className="px-6 py-4">
                                        <span className="text-sm text-slate-700">{chain.workspace}</span>
                                    </td>

                                    <td className="px-6 py-4">
                                        <span
                                            className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border ${chain.status === "Enabled"
                                        ...
                                        <div className="flex items-center gap-1 text-xs text-slate-600 max-w-xs overflow-x-auto">
                                            {chain.flow.map((step, idx) => (
                                                <React.Fragment key={`${chain.id}-flow-${idx}`}>
                                                    <span className="whitespace-nowrap font-medium">{step}</span>
                                    ...
                                    <td className="px-6 py-4">
                                        <span className="text-sm text-slate-600">{chain.createdAt}</span>
                                    </td>
                                    ...
                                    <div className="flex items-start gap-3 flex-1 min-w-0">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <div className="flex flex-wrap items-center gap-2 mt-1">
                                                ...
                                                <span
                                                    className={`inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border ${chain.status === "Enabled"
                                                        ? "bg-emerald-100 text-emerald-800 border-emerald-200"
                                                        : "bg-slate-100 text-slate-800 border-slate-200"
                                                        }`}
                                                >
