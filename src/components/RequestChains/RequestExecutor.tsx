...
import { APIRequest, ExecutionLog, Variable } from '../../types';
  ...
  const executeRequest = async (request: APIRequest, currentVars: Variable[]): Promise<ExecutionLog> => {
    ...
    try {
      ...
      if (request.authType === 'bearer' && request.authToken) {
        processedHeaders['Authorization'] = `Bearer ${replaceVariables(request.authToken, currentVars)}`;
        ...
        if (request.authApiLocation === 'header') {
          processedHeaders[request.authApiKey] = replaceVariables(request.authApiValue, currentVars);
        } else {
          url.searchParams.set(request.authApiKey, replaceVariables(request.authApiValue, currentVars));
        }
      }
