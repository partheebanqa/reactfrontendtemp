...
export default function HighlightedUrlInput({
  ...
  useEffect(() => {
    ...
    if (!ReactEditor.isFocused(editor)) {
      const currentValue = Node.string(
        slateValue[0] || { children: [{ text: '' }] }
      );
      if (currentValue !== (url || '')) {
  ...
  const renderLeaf = useCallback(
    ({ attributes, children, leaf }: any) => {
      ...
      if (leaf.variable) {
        ...
        const displayValue =
          value === 'undefined' ? 'Variable not found' : value;

        return (
          <span {...attributes} className='relative group'>
            ...
            <span className='absolute bottom-full left-0 mb-1 hidden group-hover:block bg-gray-800 text-white text-xs rounded px-2 py-1 z-20 whitespace-nowrap max-w-xs overflow-hidden text-ellipsis shadow-lg'>
              {activeEnvironment ? (
                <span className='text-blue-300'>{activeEnvironment.name}:</span>
              ) : (
                ''
              )}{' '}
              {displayValue}
