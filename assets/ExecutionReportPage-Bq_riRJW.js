...
`,Ze=e=>{const a=Oe(e.status),r=Ue(e.severity),s=`tc-${e.id}`;return`
  ...
  </div>`},We=e=>{const a=Ge(e.passed,e.total),r=a>=80?"text-green-600":a>=60?"text-yellow-600":"text-red-600",s=`url-${tt(e.key)}`,x=Object.values(e.methods).map(l=>`
    <div class="space-y-3">
      <div class="flex items-center gap-3">
        <span class="inline-flex items-center px-2 py-1 rounded text-xs font-medium border ${we(l.method)}">${l.method}</span>
      </div>
      <div class="space-y-3">
        ${l.testCases.map(n=>Ze(n)).join("")}
      </div>
    </div>`).join("");return`
  <div class="border border-gray-200 rounded-lg overflow-hidden">
    ...
    <div class="space-y-4">
      <div class="bg-white rounded-lg shadow-md p-6">
        ...
        <div class="space-y-4">${a.map(r=>We(r)).join("")}</div>
      </div>
